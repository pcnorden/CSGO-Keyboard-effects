...
namespace CSGO_Corsair {
	class DecodeJson : IPluginHost{
		...
		private bool isPluginsLoaded = false; // value to see if the plugins are loaded
		private bool areTherePlugins = false; // Value set to true if plugins exists, else false, so we don't try to call methods that don't exists
